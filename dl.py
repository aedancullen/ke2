...
def process(trackid):
    ...
    response = session.get(url, params={"search_query": trackid + " audio"}, headers=headers).text
    ...
    print("====>", trackid, topurl)
    subprocess.run('yt-dlp -f bestaudio[asr=44100] --extract-audio --audio-format mp3 --output "dl/' + trackid + '.%(ext)s" ' + topurl, shell=True, stdout=subprocess.DEVNULL)
...
with open("hot100.csv", 'r') as file:
    ...
    for line in csv:
        ...
        trackid = song + ' ' + performer
        if not year >= 2000:
            continue
        if not trackid in trackids:
